...
class TestSemver(unittest.TestCase):
    def test_should_parse_version(self):
        self.assertEqual(
            semversion.parse("1.2.3-alpha.1.2+build.11.e0f985a"),
            {'major': 1,
             'minor': 2,
             'patch': 3,
        ...
        self.assertEqual(
            semversion.parse("1.2.3-alpha-1+build.11.e0f985a"),
            {'major': 1,
             'minor': 2,
             'patch': 3,
             'prerelease': 'alpha-1',
    ...
    def test_should_raise_value_error_for_zero_prefixed_versions(self):
        self.assertRaises(ValueError, semversion.parse, "01.2.3")
        self.assertRaises(ValueError, semversion.parse, "1.02.3")
        self.assertRaises(ValueError, semversion.parse, "1.2.03")

    def test_build_version(self):
        v = "1.2.3-alpha.1.2+build.11.e0f985a"
        parse = semversion.parse(v)
        self.assertEqual(semversion.build_version(parse), v)

    def test_increase_major(self):
        v = "1.2.3"
        version = semversion.Version(v)
        version.inc_major()
